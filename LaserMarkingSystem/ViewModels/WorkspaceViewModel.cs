...
namespace InIWorkspace.ViewModels
{
    public class WorkspaceViewModel : BaseViewModel
    {
        ...
        private void InitLazerCommandExecute()
        {
            ...
            // string dll_path = $"C:\\Users\\91997\\workspace\\IZICode-Touch-Documents\\EzCAD SDK\\Ezcad3_demo\\Ezcad3_demo\\bin\\Debug";
        ...
        {
            // Start Marking
            var figures = Canvas.Figures.ToList(); // Get all figures
        ...
        private void StartRedCommandExecute()
        {
            ...
            var figures = Canvas.Figures.Where(p => p.GetType() == typeof(Rectangle));

            foreach (var item in figures.ToList())
            {
                // Check if item is of type Rectangle                
                if (item.GetType() == typeof(Rectangle))
                {
                    var err = EzdKernel.E3_CreateRect(m_idEM, 0, new Pt2d { X = item.X, Y = item.Y }, new Pt2d { X = item.X + item.Width, Y = item.Y + item.Height }, false, false, "");
                    if (err != EzdKernel.E3_ERR.SUCCESS)
                    {
                        MessageBox.Show("CreateRect=" + err.ToString());
                        return;
                    }
                }
            }
        ...
        private void backgroundWorker2_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {//计算
                nMarkFlag |= EzdKernel.MAKRFLAG_CALCTIMEMODE;
            }*/

            /* List Processing
            if (checkBox10.Checked)
            {//按列表加工,将多个对象加入控制卡加工列表
                err = EzdKernel.E3_MarkerListReady(m_idMarker, m_idEM, nMarkFlag);
                if (err != EzdKernel.E3_ERR.SUCCESS)
                {
                    MessageBox.Show("E3_MarkerListReady=" + err.ToString() + ",Err=" + nErr.ToString());
                    return;
                }
                for (int i = 0; i < m_idForListMarkEntList.Count; i++)
                {
                    err = EzdKernel.E3_MarkerMarkEntToList2(m_idMarker, m_idForListMarkEntList[i]);
                    if (err != EzdKernel.E3_ERR.SUCCESS)
                    {
                        MessageBox.Show("E3_MarkerMarkEntToList2=" + err.ToString() + ",Err=" + nErr.ToString());
                        return;
                    }
                }

                err = EzdKernel.E3_MarkerWaitForFinish(m_idMarker);
                if (err != EzdKernel.E3_ERR.SUCCESS)
                {
                    MessageBox.Show("E3_MarkerWaitForFinish=" + err.ToString() + ",Err=" + nErr.ToString());
                    return;
                }
                err = EzdKernel.E3_MarkerListEnd(m_idMarker);
                if (err != EzdKernel.E3_ERR.SUCCESS)
                {
                    MessageBox.Show("E3_MarkerListEnd=" + err.ToString() + ",Err=" + nErr.ToString());
                    return;
                }
            }
            else
            */
            ...
            //var errCode = EzdKernel.E3_GetEntityName(0, entityName);

            //if (errCode != EzdKernel.E3_ERR.SUCCESS)
            //{
            //    MessageBox.Show("GetEntityName=" + errCode.ToString());
            //    return;
            //}
